...
export default function BlogsListPage() {
  ...
  const { data: items, error, isLoading, mutate } = useSWR<Blog[]>(
    "blogs",
    (resource: string) => adminApiClient.list<Blog>(resource)
  );
  const [pendingDelete, setPendingDelete] = useState<Blog | null>(null);

  async function handleDelete() {
    if (!pendingDelete) return;
    await adminApiClient.remove("blogs", pendingDelete._id);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <AdminPageTitle>Blogs</AdminPageTitle>
        <Link
      ...
      {isLoading ? (
        ...
        <AdminTable
          ...
          onEdit={(item) => router.push(`/admin/blogs/${item._id}`)}
