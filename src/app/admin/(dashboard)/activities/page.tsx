...
export default function ActivitiesListPage() {
  ...
  const { data: items, error, isLoading, mutate } = useSWR<Activity[]>(
    "activities",
    (resource: string) => adminApiClient.list<Activity>(resource)
  );
  const [pendingDelete, setPendingDelete] = useState<Activity | null>(null);

  async function handleDelete() {
    if (!pendingDelete) return;
    await adminApiClient.remove("activities", pendingDelete._id);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <AdminPageTitle>Activities</AdminPageTitle>
        <Link
      ...
      {isLoading ? (
        ...
        <AdminTable
          ...
          columns={[
            ...
            { header: "Tag", render: (item) => item.tag },
            {
              header: "Published",
          ...
          onEdit={(item) => router.push(`/admin/activities/${item._id}`)}
