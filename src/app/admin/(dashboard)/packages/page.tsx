...
export default function PackagesListPage() {
  ...
  const { data: items, error, isLoading, mutate } = useSWR<Package[]>(
    "packages",
    (resource: string) => adminApiClient.list<Package>(resource)
  );
  const [pendingDelete, setPendingDelete] = useState<Package | null>(null);

  async function handleDelete() {
    if (!pendingDelete) return;
    await adminApiClient.remove("packages", pendingDelete._id);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <AdminPageTitle>Packages</AdminPageTitle>
        <Link
      ...
      {isLoading ? (
        ...
        <AdminTable
          ...
          onEdit={(item) => router.push(`/admin/packages/${item._id}`)}
